...
export class ImageService {
  ...
  async findAll(): Promise<Image[]> {
    return await this.imageRepository.createQueryBuilder("image").leftJoinAndSelect("image.blog", "blog").getMany();
  }

  async findOne(id) {
    return await this.imageRepository.createQueryBuilder("image").where(`image.id =${id}`).getOne();
  }
